...
| Алгоритм / задача                                                 | Тип задачи                | Как выглядит условие                                                                    | Что требуется найти                                               | Ключевые признаки в условии                                                     |
| ----------------------------------------------------------------- | ------------------------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| **Максимальное паросочетание в двудольном графе**                 | Графы                     | Есть **две группы объектов**, между ними возможны связи                                 | Максимальное число непересекающихся пар                           | двудольный граф, пары, один объект можно использовать **только один раз**       |
| **Задача о назначении (Венгерский алгоритм)**                     | Оптимизация + графы       | Есть **n исполнителей и n задач**, дана **матрица стоимости**                           | Найти назначение **1-к-1** с **минимальной суммарной стоимостью** | квадратная матрица затрат, каждому исполнителю одна задача                      |
| **Максимальный поток**                                            | Сети                      | Есть **ориентированная сеть**, источник и сток, у рёбер есть **пропускные способности** | Максимальный поток из источника в сток                            | источник (s), сток (t), пропускные способности рёбер                            |
| **Максимальный поток минимальной стоимости**                      | Сети + оптимизация        | Как в задаче потока, но у рёбер есть ещё **стоимость передачи**                         | Максимальный поток **с минимальной стоимостью**                   | поток + стоимость, нужно оптимизировать цену                                    |
| **Метод ветвей и границ (рюкзак)**                                | Комбинаторная оптимизация | Нужно выбрать **подмножество объектов** при **ограничении ресурса**                     | Максимизировать стоимость (или минимизировать)                    | перебор вариантов, ограничения (вес, объём), задача выбора подмножества         |
| **Метод ветвей и границ — задача коммивояжёра (алгоритм Литтла)** | Комбинаторная оптимизация | Дана **матрица расстояний между городами**                                              | Найти **кратчайший гамильтонов цикл**                             | нужно посетить **каждый город один раз** и вернуться в начало                   |
| **Генетический алгоритм**                                         | Эвристическая оптимизация | Очень сложная задача, полный перебор слишком дорогой                                    | Найти **приближённое оптимальное решение**                        | решения кодируются как **хромосомы**, есть **популяция, скрещивание, мутации**  |
